...
########################make a PCA plot#########################
MakPCA <- function(rt_pca, rt_sam = rt_sam, target_genes, nam = nam, phi = 40, pch = 18, len_a = len_a, len_b = len_b, 
  ...
  PCA <- scatter3D(pc$PC1, pc$PC2, pc$PC3,  xlab = xlab, ylab = ylab, zlab = zlab, bty = 'g', cex = 1.5,
                   pch  = pch, phi = phi, col = pc$color,  colvar = as.integer(pc$group), colkey = FALSE) 
    legend(len_position, legend = levels(pc$group), col = unique(pc$color), pch = 18, inset = -0.17, xpd = TRUE, horiz = TRUE, cex = 1.5)
  print(PCA)
  dev.off()
}
################################################################